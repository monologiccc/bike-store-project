...
**Воронка заказов**
- Общая конверсия "заказ создан → выполнен" — 89.5%, отклонено всего 2.8%
  заказов
- Между магазинами разброс rejection rate большой: у Rowlett Bikes — 10.9%,
  у Baldwin Bikes — 1.3% (почти в 4 раза меньше) — стоит разобраться в
  причине именно в этом магазине

**Retention**
- 75% клиентов совершают только одну покупку за весь период наблюдений —
  ожидаемо для категории товара с длинным циклом повторной покупки
- В среднем по когортам возвращается ~9% клиентов; помесячная когортная
  сетка для этой категории малоинформативна из-за редкости повторных
  покупок

**A/B-тест (синтетический эксперимент)**
- Смоделированный эффект скидки (+8% к AOV) не был статистически обнаружен
  при честном (сбалансированном) разбиении групп: p=0.800, Cohen's d≈0.013
- Причина — высокая дисперсия выручки по отдельному заказу; power-анализ
  показал, что для надёжного обнаружения эффекта такого размера
  потребовалось бы ~99,000 заказов на группу — методологический вывод о
  важности планирования размера выборки до запуска эксперимента
